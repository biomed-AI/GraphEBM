...
#			outmap.insert_scaled_sum(gaus,(a[1]/apix+xt-amin[0]/apix,a[2]/apix+yt-amin[1]/apix,a[3]/apix+zt-amin[2]/apix),res/(pi*12.0*apix),elec)
			if center:
				outmap.insert_scaled_sum(gaus,(old_div((a[1]-aavg[0]),apix)+old_div(outbox[0],2),old_div((a[2]-aavg[1]),apix)+old_div(outbox[1],2),old_div((a[3]-aavg[2]),apix)+old_div(outbox[2],2)),old_div(res,(pi*12.0*apix)),elec)
			else:
				outmap.insert_scaled_sum(gaus,(old_div(a[1],apix)+old_div(outbox[0],2),old_div(a[2],apix)+old_div(outbox[1],2),old_div(a[3],apix)+old_div(outbox[2],2)),old_div(res,(pi*12.0*apix)),elec)
